...
def simple_rnn(inputs):
    ...
    intermediate_out = tf.contrib.layers.fully_connected(rnn_out, INTERMEDIATE_SIZE_2, activation_fn=tf.nn.relu)
    final_out = tf.contrib.layers.fully_connected(intermediate_out, FINAL_OUTPUT_SIZE, activation_fn=None)
    return final_out
...
loss_op = tf.losses.huber_loss(labels=actual_Y, predictions=predicted_out)
...
with tf.Session() as sess:
    sess.run(init)
    train_writer = tf.summary.FileWriter("tensorboard/run25", sess.graph)
    ...
    for step in range(TRAINING_EPOCHS):
        ...
        while True:
            ...
            if step % SAVING_EPOCHS == 0:
                ...
                np.save("output/run25/pred_epoch" + str(step), predictions)
                np.save("output/run25/true_epoch" + str(step), true)
            break
